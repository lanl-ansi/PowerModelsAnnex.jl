...
function run_qc_opf_model(data, solver)
    model = post_qc_opf(data, Model(solver=solver))
    status = solve(model)
    return status, model
end
...
@testset "test qc w+l opf" begin
    @testset "case $(name)" for (name, case_file) in case_files
        ...
        pm_result = run_opf(data, QCWRTriPowerModel, ipopt_solver)
        pm_sol = pm_result["solution"]
        ...
        for (i, bus) in data["bus"]
            if bus["bus_type"] != 4
                ...
                #println("$i, $(getvalue(opf_model[:vm][index])), $(pm_sol["bus"][i]["vm"])")
                @test isapprox(getvalue(opf_model[:vm][index]), pm_sol["bus"][i]["vm"]; atol = 1e-6)
            end
        end

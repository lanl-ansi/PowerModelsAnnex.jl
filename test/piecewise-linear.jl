@testset "test pwl opf problems with polynomial costs" begin
    @testset "5-bus with pwl delta model" begin
        result = run_opf_pwl_delta(case_files["case5"], ACPPowerModel, ipopt_solver)

        @test result["termination_status"] == LOCALLY_SOLVED
        @test isapprox(result["objective"], 18269; atol = 1e0)
    end
    @testset "5-bus with pwl lambda model" begin
        result = run_opf_pwl_lambda(case_files["case5"], ACPPowerModel, ipopt_solver)

        @test result["termination_status"] == LOCALLY_SOLVED
        @test isapprox(result["objective"], 18269; atol = 1e0)
    end
    @testset "5-bus with pwl phi model" begin
        result = run_opf_pwl_phi(case_files["case5"], ACPPowerModel, ipopt_solver)

        @test result["termination_status"] == LOCALLY_SOLVED
        @test isapprox(result["objective"], 18269; atol = 1e0)
    end
    @testset "5-bus with pwl psi model" begin
        result = run_opf_pwl_psi(case_files["case5"], ACPPowerModel, ipopt_solver)
